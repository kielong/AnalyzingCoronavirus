...
parser_control.add_argument('--embed',action='store_true')
...
parser_infer.add_argument('--infer_path',default='coronavirus-headlines/explain_outputs')
...
class BertFineTuning(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(768,len(all_categories))
    ...
    def forward(self,x):
        ...
        return out, torch.zeros(args.input_length,args.batch_size,76)
# load the model
if args.model=='cnn':
    model = CNNModel()
...
if args.train:
    ...
    for step in range(1, args.samples + 1):
        ...
        if args.model == 'bert':
            ...
            # entire batch
            line_tensor = torch.cat([encoding['input_ids'] for encoding in encodings], dim=0)
        else:
...
def explain(line, filename, explain_type):
    ...
    if explain_type == 'char':
        for i in range(1, len(line) + 1):
            # construct input_tensor' by setting the columns associated with the word to 0
            temp_input_tensor = input_tensor.clone().detach()
            temp_input_tensor[i, 0, :] = torch.zeros([len(vocabulary)])
            output_char, _ = model(temp_input_tensor)
            temp_probs = softmax(output_char)
            scores[i - 1] = torch.dist(probs,temp_probs,2)
        ...
        for i in word_lengths:
            # construct input_tensor' by setting the columns associated with the word to 0
            temp_input_tensor = input_tensor.clone().detach()
            temp_input_tensor[count:count + i, 0, :] = torch.zeros([i, len(vocabulary)])
            output_word, _ = model(temp_input_tensor)
            temp_probs = softmax(output_word)
            word_weight = torch.dist(probs, temp_probs,2)

            for j in range(count, count + i):
                scores[j - 1] = word_weight
            count = count + i + 1
...
if args.embed:
    ...
    # print("model.fc_class.weight.shape", model.fc_class.weight.shape)
    # print("model.fc_class.bias.shape", model.fc_class.bias.shape)
    category_embedding = torch.cat([model.fc.weight, torch.unsqueeze(model.fc.bias,dim=1)],dim=1)
    print("category_embedding.shape=", category_embedding.shape)
